...
class Config(StudioTab):
    ...
    def __init__(self, studio):
        ...
        if self.nanohub_flag or self.studio.galaxy_flag:
    ...
    def random_seed_gp_cb(self, id):
        ...
        if hasattr(self.studio, 'user_params_tab'):
            # this is called before user_params_tab is created one time, so make sure it exists
            for idx in range(self.studio.user_params_tab.count):
                v_name = self.studio.user_params_tab.utable.cellWidget(idx,self.studio.user_params_tab.var_icol_name).text()

...
class Config(StudioTab):
    ...
    def __init__(self, p_studio):
        super().__init__(p_studio)
        ...
        if self.nanohub_flag or self.p_studio.galaxy_flag:
    ...
    def random_seed_gp_cb(self, id):
        ...
        if hasattr(self.p_studio, 'user_params_tab'):
            # this is called before user_params_tab is created one time, so make sure it exists
            for idx in range(self.p_studio.user_params_tab.count):
                v_name = self.p_studio.user_params_tab.utable.cellWidget(idx,self.p_studio.user_params_tab.var_icol_name).text()
